<template>
  <div class="rotationDialog">
    <el-dialog width="55%" title="修改主页轮播图" :visible.sync="dialogAdd" :close-on-click-modal="false" :before-close="handleClose">
      <el-form
        :model="addInfo"
        label-width="100px"
        :rules="rules"
      ...
      >
        ...
        <!-- 放示例图 -->
        <el-form-item label="效果图"  v-show="addInfo.type == 1">
          <img src="../../../assets/solo.png" alt="" style="height:150px;">
        </el-form-item>
        ...
        <el-form-item label="一级分类" prop="parentCateCode" v-show="addInfo.type == 3">
          <el-select v-model="addInfo.parentCateCode" @change="firstCategoryChange(1)" placeholder="请选择">
            <el-option
...
export default {
  ...
  methods: {
    ...
    openDialog(flag,val) {
      this.dialogAdd = flag;
      this.selectInfo = val.bannerCode;
      this.$nextTick(() => {
    ...
    getDetail(){
      let params = {
        bannerCode:this.selectInfo
      };
      this.loading.startLoading();
        this.$axios.post(API.DETAIL_ROTATION_API, params).then(res => {
        this.loading.endLoading();
          if (0 == res.data.code) {
            this.addInfo = res.data.data;
            this.fileListPic[0].url = res.data.data.imageUrl;
            this.uploadData = res.data.data.goods;
            this.firstCategoryChange();
          } else {
            this.$message({
            showClose: true,
    ...
    addSure() {
      this.$refs['addInfo'].validate((valid) => {
        if (valid) {
          ...
          this.loading.startLoading();
          this.$axios.post(API.UPDATE_ROTATION_API, this.addInfo).then(res => {
            this.loading.endLoading();
            if (0 == res.data.code) {
              this.$message({
                showClose: true,
    ...
    handleRemovePic(file, fileList) {
      let params = {
        filePath: file.response == undefined ? file.url: file.response.data
      };
