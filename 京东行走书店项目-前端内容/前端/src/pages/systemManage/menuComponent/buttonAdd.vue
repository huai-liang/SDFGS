...
export default {
  ...
  methods: {
    openDialog(flag, menu) {
      ...
      this.menu = menu;
      this.$nextTick(() => {
        this.$refs.addButtonList.resetFields();
      });
    },
    cancelDialog() {
    ...
    addButtonSure() {
      this.$refs["addButtonList"].validate((valid) => {
        if (valid) {
          this.addButtonList.menuCode = this.menu.menuCode;
          this.addButtonList.parentButtonCode = "0";
          this.addButtonList.level = "1";
          this.loading.startLoading();
          this.$axios.post(API.ADD_MENU_BTN_API, this.addButtonList).then(res => {
