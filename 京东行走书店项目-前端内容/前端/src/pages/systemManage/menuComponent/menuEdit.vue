    ...
    <el-dialog title="新增菜单" :visible.sync="dialogMenuAdd" width="30%" :close-on-click-modal="false">
      ...
      >
        ...
        <el-form-item
          label="方法"
          prop="menuUrl"
          v-if="addMenuList.type==='1'||addMenuList.type==='菜单'"
        >
  ...
  methods: {
    ...
    addMenuSure() {
      this.$refs["addMenuList"].validate((valid) => {
        if (valid) {
          if ('菜单' == this.addMenuList.type || 1 == this.addMenuList.type) {
            this.addMenuList.type = 1;
          }
          if ('目录' == this.addMenuList.type || 2 == this.addMenuList.type) {
            this.addMenuList.type = 2;
          }
          this.loading.startLoading();
          this.$axios.post(API.UPDATE_MENU_API, this.addMenuList).then(res => {
